...
export default function ApplyModal({ cargo, onClose }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const caminho = `${cargo.id}/${Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 8)}.${extensao}`
      ...
      >
        <div className="bg-armac-blue rounded-t-2xl px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <h2 className="text-white font-bold text-lg leading-tight">{cargo.titulo}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        {enviado ? (
          <div className="p-8 text-center">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Recebemos seu currículo para a vaga de {cargo.titulo}. Nossa equipe vai
              analisar e entrar em contato caso o perfil combine com a vaga.
            </p>
            <button
              type="button"
              onClick={onClose}
